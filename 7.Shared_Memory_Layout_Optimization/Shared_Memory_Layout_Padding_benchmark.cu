...
constexpr size_t SHARED_MEMORY_BYTES =
    static_cast<size_t>(BK) * (AS_STRIDE + BS_STRIDE) * sizeof(float);
...
struct Config
{
    ...
    int warmup = 10;
    int iterations = 50;
    int max_check_dim = 1024;
    unsigned int seed = 20260717u;
...
void usage(const char *program)
{
    std::cout
        ...
        << "  --max-check-dim <int>   CPU-check size limit (default 1024)\n"
        << "  --seed <uint>           input seed\n"
        << "  --no-check              disable CPU correctness check\n"
        << "  --csv                   print machine-readable output\n"
        << "  --help                  show this help\n\n"
        << "Kernel variant: As/Bs shared-memory padding\n"
...
int main(int argc, char **argv)
{
    ...
    if (!config.csv)
    {
        std::cout << "Kernel: sgemm_shared_memory_layout_padding\n"
                  << "Layout: As[BK][BM+" << AS_PADDING << "], Bs[BK][BN+"
