...
void printUsage(const char *prog)
{
    cout << "Usage:\n"
         ...
         << "  --bx <int>           blockDim.x (must be 512 for this kernel)\n"
    ...
    float *__restrict__ C)
{
    ...
    // ------------------------------------------------------------
    // 每个线程负责 C tile 中的一列 + 连续 TM 行
    // 一个 block 负责 BM x BN = 64 x 64 个 C 元素
    // 512 个线程，每个线程算 8 个结果
    // ------------------------------------------------------------
    const int threadCol = tid % BN;      // 0 ~ 63
    const int threadRowGroup = tid / BN; // 0 ~ 7

    const int localRowBase = threadRowGroup * TM;
    const int globalCol = blockCol + threadCol;
    const int globalRowBase = blockRow + localRowBase;

    // ------------------------------------------------------------
    // A tile: BM x BK = 64 x 8 = 512
    // B tile: BK x BN = 8 x 64 = 512
    // 正好每个线程加载 A 的一个元素和 B 的一个元素
    // ------------------------------------------------------------
    const int aTileRow = tid / BK; // 0 ~ 63
    const int aTileCol = tid % BK; // 0 ~ 7

    const int bTileRow = tid / BN; // 0 ~ 7
    const int bTileCol = tid % BN; // 0 ~ 63

    float threadResult[TM];
...
#pragma unroll
    ...
    // 判断当前 block 是否完整落在矩阵内部
    // 对 1024 / 2048 / 4096 这种整齐尺寸，大部分 block 都走无边界检查路径
    const bool fullBlockMN = (blockRow + BM <= M) && (blockCol + BN <= N);

    for (int bkIdx = 0; bkIdx < K; bkIdx += BK)
    {
        const bool fullKTile = (bkIdx + BK <= K);
        ...
        {
            As[aTileRow * BK + aTileCol] =
                A[(blockRow + aTileRow) * K + (bkIdx + aTileCol)];

            Bs[bTileRow * BN + bTileCol] =
                B[(bkIdx + bTileRow) * N + (blockCol + bTileCol)];
        }
        else
        {
        ...
        // --------------------------------------------------------
        // 计算当前 BK tile 对 threadResult[0~7] 的贡献
        // --------------------------------------------------------
#pragma unroll
...
#pragma unroll
            ...
            {
                const int localRow = localRowBase + resIdx;

                threadResult[resIdx] +=
                    As[localRow * BK + dotIdx] * bValue;
            }
        }
...
#pragma unroll
        for (int resIdx = 0; resIdx < TM; ++resIdx)
        {
            C[(globalRowBase + resIdx) * N + globalCol] =
                threadResult[resIdx];
        }
    }
    else
...
BenchmarkConfig parseArgs(int argc, char **argv)
{
    ...
    {
        cerr << "1D blocktiling kernel expects blockDim = (" << THREADS_PER_BLOCK << ", 1)" << endl;
        exit(1);
    }
